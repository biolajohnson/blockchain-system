...
// converts the transaction the main server sent into a string an appends it to the file/block2.txt
int process_and_save_transaction_entry(char*command){
	string line(command);
	ofstream myfile;
	myfile.open("block2.txt", ios::app);  // opens the text file to append to it.
	myfile << line << endl;
	myfile.close();
	return 0;
}
...
int main(int argc, char* argv[]){
	...
	local_addr.sin_port = htons(22702); // port number
	...
	cout << "The ServerB is up and running using UDP on port " << port_no << endl;
