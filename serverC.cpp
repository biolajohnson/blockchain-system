...
// converts the transaction the main server sent into a string an appends it to the file/block3.txt
int process_and_save_transaction_entry(char*command){
	string line(command);
	ofstream myfile;
	myfile.open("block3.txt", ios::app); // opens the text file to append to it .
	myfile << line << endl;
	myfile.close();
	return 0;
}
...
int main(int argc, char* argv[]){
	...
	local_addr.sin_port = htons(23702); // port number
	...
	cout << "The ServerC is up and running using UDP on port " << port_no << endl;
